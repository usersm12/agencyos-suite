-- ============================================================
-- Add p_is_multisite to update_client RPC so existing clients
-- can be converted to multi-property from the Edit Client modal.
-- ============================================================
CREATE OR REPLACE FUNCTION public.update_client(
  ...
  p_is_multisite            BOOLEAN    DEFAULT NULL   -- NULL = don't change
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSIF v_role = 'manager' THEN
    ...
    END IF;
    -- Managers cannot change the multisite flag
    IF p_is_multisite IS NOT NULL THEN
      RETURN jsonb_build_object('error', 'Forbidden: only owners can change multisite setting');
    END IF;
  ELSE
  ...
  UPDATE public.clients SET
    ...
    notes                  = p_notes,
    -- only update is_multisite when explicitly passed
    is_multisite           = COALESCE(p_is_multisite, is_multisite),
    updated_at             = now()
...
GRANT EXECUTE ON FUNCTION public.update_client(UUID,TEXT,TEXT,TEXT,TEXT,TEXT,TEXT,TEXT,DATE,NUMERIC,TEXT,UUID,TEXT,TEXT,BOOLEAN)
  TO authenticated;
